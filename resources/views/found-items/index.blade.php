...
<div class="dashboard-wrapper">
    ...
    <div class="page-header">
        <div class="header-left">
            ...
            <p>{{ $isAdmin ? 'Manage and verify reported found items' : 'Browse all items that have been found' }}</p>
        </div>
    ...
    <div class="stats-grid">
           ...
           class="stats-link">
            <div class="stat-card">
                ...
                <div class="stat-content">
                    ...
                    <div class="stat-label">{{ $isAdmin ? 'Pending Approval' : 'Pending' }}</div>
                </div>
                @if($isAdmin && $pendingCount > 0)
        ...
        <a href="{{ route('found-items.index', array_merge(request()->query(), ['status' => 'approved', 'category' => ''])) }}" 
           class="stats-link">
            <div class="stat-card">
        ...
        <a href="{{ route('found-items.index', array_merge(request()->query(), ['status' => 'rejected', 'category' => ''])) }}" 
           class="stats-link">
            <div class="stat-card">
        ...
        <a href="{{ route('found-items.index', array_merge(request()->query(), ['status' => 'returned', 'category' => ''])) }}" 
           class="stats-link">
            <div class="stat-card">
        ...
        <a href="{{ route('found-items.index', array_merge(request()->query(), ['status' => 'disposed', 'category' => ''])) }}" 
           class="stats-link">
            <div class="stat-card">
                <div class="stat-icon" style="background: linear-gradient(135deg, #666666, #888888);">
                    <i class="fas fa-times"></i>
                </div>
                <div class="stat-content">
    ...
    <div class="custom-alert info-alert mb-4">
        <div class="alert-content">
                ...
                <div class="filter-tags">
                    @if(request('status'))
                        <span class="filter-tag">
                            Status: {{ request('status') == 'pending' ? ($isAdmin ? 'Pending Approval' : 'Pending') : (request('status') == 'approved' ? 'Approved' : (request('status') == 'rejected' ? 'Rejected' : (request('status') == 'claimed' ? 'Claimed' : (request('status') == 'returned' ? 'Returned' : 'Disposed')))) }}
                        </span>
                    @endif
                    @if(request('category'))
                        <span class="filter-tag">
            ...
            <a href="{{ route('found-items.index') }}" class="alert-action-btn">
                ...
                <div class="filter-form-row">
                    ...
                    <div class="filter-form-group filter-actions-group">
                        <div class="filter-actions">
                            ...
                            <a href="{{ route('found-items.index') }}" class="btn btn-outline-primary w-100">
    ...
    @if($isAdmin && !in_array(request('status'), ['approved', 'rejected', 'claimed', 'returned', 'disposed']))
                ...
                <div class="items-grid-col">
                    <div class="item-card pending-card">
                        <div class="item-card-header">
                            ...
                            <div class="item-actions-dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li>
                                        <form action="{{ route('found-items.approve', $item) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="dropdown-item text-success" onclick="return confirm('Approve this found item? It will be visible to all users.')">
                                                <i class="fas fa-check-circle"></i> Approve
                                            </button>
                                        </form>
                                    </li>
                                    <li>
                                        <form action="{{ route('found-items.reject', $item) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="dropdown-item text-danger" onclick="return confirm('Reject this found item? The user will be notified.')">
                        ...
                        <div class="item-card-body">
                            ...
                            <div class="item-location" title="{{ $item->found_location }}">
                                <i class="fas fa-map-marked-alt"></i>
                                {{ \Illuminate\Support\Str::limit($item->found_location, 25) }}
                            </div>
                            @endif
                            
                            <div class="admin-actions">
                                <form action="{{ route('found-items.approve', $item) }}" method="POST" class="flex-1">
                                    @csrf
                                    <button type="submit" class="btn btn-success w-100">
                                        <i class="fas fa-check"></i> Approve
                                    </button>
                                </form>
                                <form action="{{ route('found-items.reject', $item) }}" method="POST" class="flex-1">
                                    @csrf
    ...
    <div class="approved-items-section">
        <div class="section-header">
            <h3>
                <i class="fas {{ $isAdmin ? 'fa-check-circle' : 'fa-box-open' }}" style="color: var(--success);"></i> 
                {{ $isAdmin ? 'All Items' : 'Found Items' }}
            </h3>
        </div>
            ...
            <div class="items-grid-col">
                <div class="item-card">
                        ...
                        <div class="item-actions-dropdown">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li>
                                    <form action="{{ route('found-items.reject', $item) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="dropdown-item text-danger" onclick="return confirm('Reject this approved item?')">
                                            <i class="fas fa-times-circle"></i> Reject
                                        </button>
                                    </form>
                                </li>
                    ...
                    <div class="item-card-body">
                        ...
                        <a href="{{ route('found-items.show', $item) }}" class="view-details-btn">
                            <i class="fas fa-eye"></i> View Details
                    ...
                    <div class="item-card-footer">
                        ...
                        @endif
                        @if($item->rejected_at)
                        <span class="rejected-badge">Rejected {{ $item->rejected_at->diffForHumans() }}</span>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="empty-state-wrapper">
                <div class="empty-state">
                    ...
                    <p>
                        ...
                        @else
                            No found items have been reported yet. Be the first to report one!
                        @endif
                    </p>
                    @if(!$isAdmin)
        ...
        <div class="quick-stats-grid">
            <a href="{{ route('found-items.index', ['status' => 'pending']) }}" class="quick-stat-item">
                ...
                <div class="quick-stat-info">
                    ...
                    <div class="quick-stat-label">{{ $isAdmin ? 'Pending Approval' : 'Pending' }}</div>
                </div>
            </a>
            ...
            <a href="{{ route('found-items.index', ['status' => 'rejected']) }}" class="quick-stat-item">
                <div class="quick-stat-icon" style="background: linear-gradient(135deg, #ff4444, #ff6b6b);">
                    <i class="fas fa-times-circle"></i>
                </div>
                <div class="quick-stat-info">
                    <div class="quick-stat-number">{{ $rejectedCount ?? 0 }}</div>
            ...
            <a href="{{ route('found-items.index', ['status' => 'claimed']) }}" class="quick-stat-item">
                ...
                <div class="quick-stat-info">
                    <div class="quick-stat-number">{{ $claimedCount }}</div>
                    <div class="quick-stat-label">Claimed</div>
                </div>
            </a>
        </div>
    </div>
